...
namespace Xunit
{
    ...
    public class ExecutorWrapper : IExecutorWrapper
    {
        ...
        object CreateObject(string typeName, params object[] args)
        {
            try
            {
                return appDomain.CreateInstanceAndUnwrap(xunitAssemblyName.FullName, typeName, ignoreCase: false, bindingAttr: BindingFlags.Default,
                                                         binder: null, args: args, culture: null, activationAttributes: null);
            }
            catch (TargetInvocationException ex)
            {
                RethrowWithNoStackTraceLoss(ex.InnerException);
        ...
        public class IntCallbackHandlerWithIMessageSink : IntCallbackHandler, IMessageSink
        {
            ...
            IMessage IMessageSink.SyncProcessMessage(IMessage msg)
            {
                if (msg is IMethodCallMessage)
                    return System.Runtime.Remoting.RemotingServices.ExecuteMessage(this, (IMethodCallMessage)msg);
                
                object value = msg.Properties["data"];
                Result = Convert.ToInt32(value);
                return new OutgoingMessage123(true);
            }
        }
        ...
        public class XmlNodeCallbackHandlerWithIMessageSink : XmlNodeCallbackHandler, IMessageSink
        {
            ...
            IMessage IMessageSink.SyncProcessMessage(IMessage msg)
            {
                ...
                return new OutgoingMessage123(@continue);
            }
        }
        ...
        /// THIS CLASS IS FOR INTERNAL USE ONLY.
        /// </summary>
        public class OutgoingMessage123 : MarshalByRefObject, IMessage
        {
            Hashtable values = new Hashtable();

            /// <summary/>
            public OutgoingMessage123(object value)
            {
                values["data"] = value;
            }
